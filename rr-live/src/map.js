var map = {
	roles: [],
	registerRole: function(name, key) {
		var role = new Role(name);
		map.roles[key] = role;
	},
	init: function() {
		map.registerRole("toplane", "TOP");
		map.registerRole("jungle", "JUNGLE");
		map.registerRole("midlane", "MID");
		map.registerRole("adc", "ADC");
		map.registerRole("support", "SUPPORT");
	},
	ui: {
		elements: {
...
function Role(name) {
	...
	var hide = function() {
		...
		setTimeout(function() {if ((overlay.currentStyle || window.getComputedStyle(overlay)).opacity == "0") overlay.style.visibility = "hidden";}, 300);
	};
